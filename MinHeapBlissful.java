...
public class MinHeapBlissful {
    ...
    public void insert(Song value) {
        ...
        while(hole > 1 && value.blissfulScore <= array.get(hole/2).blissfulScore ) {
            if (value.blissfulScore==array.get(hole/2).blissfulScore){
    ...
    public void delete(Song song){
        ...
        if (size>1) {
            percolateDown(hole);
            while (hole > 1 && array.get(hole).blissfulScore <= array.get(hole / 2).blissfulScore) {
                if (array.get(hole).blissfulScore == array.get(hole / 2).blissfulScore) {
    ...
    private void percolateDown(int hole) {
        ...
        while(hole * 2 <= size) {
            ...
            if(child != size && array.get(child + 1).blissfulScore <= array.get(child).blissfulScore) {
                if (array.get(child+1).blissfulScore == array.get(child).blissfulScore){
